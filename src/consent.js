...
//
// This replaced a self-hosted banner that existed only for the PropellerAds
// period, when there was no AdSense tag to carry a CMP. With AdSense back, that
// banner is not merely redundant but wrong: an uncertified prompt cannot lawfully
// gate AdSense in the EEA/UK, and two prompts would be worse than one.
//
// It also restores the other half of what googlefc does. `index.html` defaults
// analytics_storage to denied across the EEA/UK/CH; Google's message is what
// flips it to granted there, so with the CMP back no gtag('consent','update')
// call of our own is needed to keep Analytics alive in Europe.
//
