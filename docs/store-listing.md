...
- **Every claim is checkable in the app.** The mastery threshold, the deck and
  ...
  with them.
- **The collection sizes are floors, not counts.** "330+" for the whole book and
  "280+" for what Pro adds, so shipping a new recipe does not silently make the
  listing wrong. `src/cocktails.json` holds 334 as of this writing (50 free +
  284 Pro); raise each floor to the next round number once the collection passes
...
```
Learn 330+ classic cocktail recipes by heart with flashcards and drill quizzes.
```
...
Cocktail Flashcards drills the ingredients, ratios, glassware and serve of 330+ classic cocktails — the Drinks International Bestselling Classics list, plus the deeper cuts a back bar eventually gets asked for — until you can call them from memory in the middle of a Friday night rush.
...
Search all 330+ recipes by name, accent-insensitive, so "pina" finds the Piña Colada. Read any spec and add drinks straight to your deck.
...
Free: the top 50 bestselling classics in Study Mode and both quizzes, the full index of 330+ recipes to read and search, supported by ads.

Cocktail Flashcards Pro — $7.99, paid once, not a subscription: adds all 280+ cocktails to Study Mode and both quizzes, and removes the ads. Buy it in the app and it works on the website too under the same Google account, and it restores on a new device.
